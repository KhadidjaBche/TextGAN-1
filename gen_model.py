...
class GenModel():
    ...
    def generate(self):
        inputs = tf.split(axis=1, num_or_size_splits=self.args.seq_length, value=tf.nn.embedding_lookup(self.embedding, self.input_data))
        ...
        with tf.variable_scope('GEN', reuse=self.has_init_seq2seq) as scope:
            ...
            if self.args.num_layers == 1:
                outputs, last_state = legacy_seq2seq.rnn_decoder(inputs, [self.initial_state1], self.cell, loop_function=loop, scope=scope)
            elif self.args.num_layers == 2:
                outputs, last_state = legacy_seq2seq.rnn_decoder(inputs, [self.initial_state1, self.initial_state2], self.cell, loop_function=loop, scope=scope)
            else:
