...
class DiscModel():
    ...
    def attach_cost(self, gen_model):
        ...
        self.loss_text = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=predicted_classes_text, labels=np.ones((self.args.batch_size, 1), dtype=np.float32)))
        generated_wv = gen_model.generate()
        predicted_classes_wv = self.discriminate_wv(generated_wv)
        self.loss_gen = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=predicted_classes_wv, labels=np.zeros((self.args.batch_size, 1), dtype=np.float32)))
    ...
    def discriminate_wv(self, input_data_wv):
        with tf.variable_scope('DISC', reuse=self.has_init_seq2seq) as scope:
            ...
            output_wv, states_wv = legacy_seq2seq.rnn_decoder(input_data_wv, self.initial_state, self.cell, scope=scope)
    ...
    def discriminate_text(self, input_data_text):
        inputs = tf.split(axis=1, num_or_size_splits=self.args.seq_length, value=tf.nn.embedding_lookup(self.embedding, input_data_text))
